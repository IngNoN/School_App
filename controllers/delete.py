...
from models import DepartmentManager, GroupOfTeacher, Teacher, db
...
def delete_department_manager():
    ...
    if DeleteItemData.validate_on_submit:
        ...
        teacher_to_delte = db.session.query(DepartmentManager).filter(DepartmentManager.departmentManager_Id == department_manager_Id_to_delete)
        teacher_to_delte.delete()
        db.session.commit()
...
def delete_group_of_teachers():
    ...
    if DeleteItemData.validate_on_submit:
        ...
        teacher_to_delte = db.session.query(GroupOfTeacher).filter(GroupOfTeacher.group_of_teachers_Id == group_of_teachers_Id_to_delete)
        teacher_to_delte.delete()
        db.session.commit()
